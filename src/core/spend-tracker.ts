...
export class SpendTracker {
   ...
   */
  async previewRecord(agentId: string, amount: number | null): Promise<SpendRecordResult> {
   ...
   */
  async record(agentId: string, amount: number | null, maxWindowMillis?: number): Promise<SpendRecordResult> {
    return this.mutex.runExclusive(async () => {
  ...
  /**
    ...
    */
  async checkWindow(agentId: string, windowConfig: SpendWindowConfig, amount = 0): Promise<WindowCheckResult> {
    return this.mutex.runExclusive(async () => {
  ...
  async getStatus(agentId: string): Promise<{
    ...
    if (this.config?.window) {
      const wRes = await this.previewCheckWindow(agentId, this.config.window);
